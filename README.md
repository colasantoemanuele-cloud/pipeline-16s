...
con `pytest`. R non serve per questa via: i calcoli scientifici girano nell'immagine
descritta qui sotto, che porta con sé il proprio R.
...
- il catalogo degli errori: ogni codice porta un messaggio che dice cosa fare e una
  ...
  codice che solleverà quegli errori no;
...
- la catena di integrazione continua, che a ogni push installa il pacchetto con
  Python 3.11 ed esegue la suite di test;
